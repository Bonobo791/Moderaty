...
<div class="table-wrap">
	<table>
		...
		<tbody>
			...
			<tr>
				<td>Service operation: moderation configuration, moderation outcome records (comment ID reference, verdict, action, timestamp — no comment bodies)</td>
...
<p><strong>3.2</strong> We operate a process-and-discard architecture: comment text and author identifiers are retrieved via the YouTube API Services, classified, acted upon according to the channel owner's configuration, and immediately discarded. We retain only moderation outcome records (comment ID reference, verdict, action, timestamp). We do not build profiles of comment authors, we do not perform author-level behavioral analysis, and we do not train models on comment data.</p>
