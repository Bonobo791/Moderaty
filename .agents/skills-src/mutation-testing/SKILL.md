...
StrykerJS is the tool for every mutation run in this repo: `@stryker-mutator/core` + `@stryker-mutator/vitest-runner`, configured by `stryker.config.json` at the repo root (vitest runner, `mutate` globs covering `src/**/*.ts` minus tests/dev-seed/legal pages and the policy-excluded `src/lib/auto-refresh.svelte.ts` — vitest compiles `.svelte.ts` for SSR, where `$effect` is a no-op, so its mutants are unreachable in this harness; its SSR no-op contract is test-pinned, client behavior is e2e territory. `scripts/stryker-pr-scope.mjs`'s `EXCLUDED_FILES` must mirror the config's mutate globs whenever the exclusions change, `coverageAnalysis: perTest`, `incremental`, `progress`/`clear-text`/`html`/`json` reporters — the `json` report at `reports/mutation/mutation.json` is the machine-readable survivor list for the agent loop).
