...
## 6. Stripe Tax for digital goods/credits
...
## 7. Official SDK vs raw REST; serverless & SvelteKit gotchas
...
- **v1 scope (this codebase)**: credits are reversed only on a **full** refund — the webhook subscribes to `charge.refunded` and reverses the entire grant, and `applyLedgerDelta` floors refund reversals at zero (a refunded grant zeroes the balance; dispute reversals keep a true negative so a won-dispute restore re-credits correctly). A full refund on a **subscription** charge also cancels the subscription at Stripe — otherwise it stays active and the next `invoice.paid` would grant a fresh paid period on a refunded account. Partial refunds (`refund.created`/`refund.updated`) are intentionally **not** handled — a deliberate v1 limitation; revisit before supporting partial refunds.

## 9. Recommended design

**Events to subscribe**: `checkout.session.completed`, `checkout.session.async_payment_succeeded`, `checkout.session.async_payment_failed` (optional), `charge.refunded`, `refund.updated`, `refund.failed`, `charge.refund.updated`, `charge.dispute.created` (+ optionally `charge.dispute.closed`, `charge.dispute.funds_withdrawn`, `charge.dispute.funds_reinstated`). Route: `src/routes/api/stripe/webhook/+server.ts` — verify raw body (`await request.text()` + `constructEvent`), return 2xx fast, run `fulfill_checkout(session.id)` idempotently. `refund.updated`/`charge.refund.updated` are NOT for partial-refund reversal (still v1-out-of-scope) — they carry the terminal status of the app's own ungrantable refunds (tagged `metadata.reason='ungrantable'`, `org_id`, and `checkout_session_id` when session-backed, at create). `refund.updated` is the event Stripe emits for ALL payment methods — `charge.refund.updated` fires only for selected ones — and `refund.failed` covers a refund that is created already failed, so the dispatcher handles all three. A `failed`/`canceled` outcome on an ACKed pending refund means the customer is still charged: the handler marks the checkout attempt `manual_refund_required` (durable, queryable), screams MANUAL REFUND REQUIRED, and throws so the event stays in Stripe's failed-delivery queue — the operator surface for PI-only auto-top-up refunds that have no attempt row.
...
CREATE TABLE stripe_events (
  ...
  processed_at TEXT,
  -- No UNIQUE(event_type, object_id): distinct events for one object must run
);
...
Insert `stripe_events` in the same transaction as the grant; the UNIQUE `event_id` constraint makes repeated deliveries safe. Distinct events for one object remain processable (https://docs.stripe.com/webhooks — "record the event IDs you've processed").
