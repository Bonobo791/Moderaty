...
4. **Environment Variables** (same ten keys as the Netlify production
   ...
   **Expired Turso tokens fail loudly now.** The gate's first step is
   `scripts/db-preflight.mjs`, a `SELECT 1` through the same `@libsql/client`
   ...
   database — ... HTTP status 401`, mint a non-expiring token
   (`turso db tokens create dev-2 -e never`) and update every copy: the
