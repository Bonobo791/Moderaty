...
4. **Environment Variables** (same ten keys as the Netlify production
   ...
   | `STRIPE_TEST_PRODUCT` | optional | optional | a Product (`prod_…`) or Price (`price_…`) id in the app's own Stripe environment — enables the owner-only "Test checkout" card that smoke-tests the billing pipeline with a real purchase (grants 1 credit) |
