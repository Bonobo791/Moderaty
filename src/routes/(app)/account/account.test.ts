...
function loadAccount(user: typeof OWNER | null = OWNER) {
...
test('account load returns the user facts, role, and the team channel count', async () => {
	...
	expect(data).toEqual({
		user: { displayName: OWNER.displayName, email: OWNER.email },
		channelCount: 2,
		maintenance: false,
		orgRole: 'owner'
	});
	// Nothing beyond the projected facts is serialized (no tokens, no ids).
	expect(JSON.stringify(data)).not.toContain('enc');
});

test('account load rejects a signed-out request with 401', async () => {
	await expect(loadAccount(null)).rejects.toMatchObject({ status: 401 });
});
...
test('a database outage returns the maintenance payload without requiring a user', async () => {
	...
	// trip — the maintenance state replaces the page, not the error page.
	const data = (await load({ locals: { user: null, dbDown: true } } as never)) as Record<string, unknown>;
	expect(data).toEqual({ user: null, channelCount: 0, maintenance: true, orgRole: null });
});
...
test('a database failure mid-load degrades to the maintenance payload and logs loudly', async () => {
	...
	expect(data).toEqual({ user: null, channelCount: 0, maintenance: true, orgRole: null });
	expect(console.error).toHaveBeenCalledWith('account load failed:', expect.any(Error));
});
...
// ── deleteAccount (moved from the dashboard — contract unchanged) ────
